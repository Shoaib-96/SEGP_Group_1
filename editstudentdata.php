...
<div class="w3-main" style="margin-left:283px">
  ...
  <footer id="myFooter">
	...
	<?php
			  ...
			  echo"<input type=\"text\" name=\"uyear\" value=\"".$rows3['year']."\" placeholder=\"Enter Year\" >";
			  echo"<br>";
			  ...
			  echo"<br>";
			  echo "<label><b>PATs</b></label><br>";
					echo "<select name=\"pat\" id=\"pat\" style=\"width:100%\" '>";
					echo "<option  value=\"".$rows3['pat_name']."\">".$rows3['pat_name']."</option>";
					$sql4="SELECT name from pats";
					$result4=$db->query($sql4);
						while($rows4=$result4->fetch_assoc()){
								if(($rows4['name']!=$rows3['pat_name']) && ($rows4['name']!="Registrar")){
								echo "<option  value=\"".$rows4['name']."\">".$rows4['name']."</option>";
							}
						}
			  echo "</select>";
   ...
   ?>
	...
	<?php
		...
		while($rows1=$result1->fetch_assoc()){
				...
				echo "</form>";
				echo "<div class=\"w3-half\"><button class=\"btn-danger\" onclick=\"document.getElementById('".$rows1['email']."').style.display='none'\" class=\"close\" title=\"Close Modal\" >No</button></div>";
				echo "</div></div>";
